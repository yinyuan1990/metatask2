<template>
    <div class="add-asset-page">
      <!-- 头部固定 -->
      <div class="header">
        <div class="title">{{ $t('addAsset.title') }}</div>
        <img
          src="@/static/icon/close.svg"
          alt="close"
          class="close-icon"
          :title="$t('addAsset.close')"
          @click="closePage"
        />
      </div>
  
      <!-- tabbar 固定底部 -->
      <div class="tabbar">
        <div
          :class="['tab-item', activeTab === 'search' ? 'active' : '']"
          @click="switchTab('search')"
        >
          {{ $t('addAsset.tabs.search') }}
        </div>
        <div
          :class="['tab-item', activeTab === 'custom' ? 'active' : '']"
          @click="switchTab('custom')"
        >
          {{ $t('addAsset.tabs.customToken') }}
        </div>
      </div>
  
      <!-- 下面放两个默认 slot，方便你后续替换成组件 -->
      <div class="tab-content">
        <!-- 改为 -->
        <div class="tab-content">
        <div v-if="activeTab === 'search'">
          <Coinsearch
            @selectNetClick="onSelectNetClick"
            @nextClick="onNextClick"
          />
        </div>
        <div v-if="activeTab === 'custom'">
          
          <CoinsearchZdy
            @selectNetClick="onSelectNetClick"
            @nextClick="onNextClick"
          />


        </div>
      </div>



      </div>
    </div>
  </template>
  
  
  <script>

import Coinsearch from '@/components/common/Coinsearch.vue'
import CoinsearchZdy from '@/components/common/CoinsearchZdy.vue'

  export default {

    name: 'AddAssetPage',
    components: {
        Coinsearch,
        CoinsearchZdy
  },
    data() {
      return {
        activeTab: 'search',
      }
    },
    methods: {
      switchTab(tab) {
        this.activeTab = tab;
      },
      closePage() {
        this.$router.back();
      },
      onSelectNetClick() {
      // 这里写点击选链时的逻辑，比如弹出选链弹窗，或者跳转选链页
      console.log('选链按钮点击了')
    },
    onNextClick(selectedToken) {
      // 选择代币后点击下一步，selectedToken 是选中的代币对象
      console.log('选择代币:', selectedToken)
      // 这里可以继续后续逻辑，比如保存选中代币、跳转下一步等
      this.$router.back()
    }
    }
  }
  </script>
  
  <style scoped>
  .add-asset-page {
    display: flex;
    flex-direction: column;
    height: 100vh;
    background: #fff;
    padding-top: 60px; /* 给头部留空间 */
    padding-bottom: 50px; /* 给tabbar留空间 */
  }
  
  /* 头部固定 */
  .header {
    position: fixed;
    top: 0;
    left: 0;
    right: 0;
    height: 60px;
    background: #fff;

    display: flex;
    align-items: center;
    justify-content: center;
    padding: 0 40px 0 20px;
    box-sizing: border-box;
    z-index: 1000;
  }
  .title {
    font-weight: 700;
    font-size: 20px;
    color: #000;
    flex: 1;
    text-align: center;
  }
  .close-icon {
    width: 20px;
    height: 20px;
    cursor: pointer;
    position: absolute;
    right: 20px;
    top: 20px;
  }
  
  /* tabbar固定底部 */
  .tabbar {
  margin: 20px;
  display: flex;
  height: 50px;
  border-bottom: 1px solid #ccc;
  background: white;
}

.tab-item {
  flex: 1;
  text-align: center;
  line-height: 50px;
  color: gray;
  cursor: pointer;
  user-select: none;
  transition: color 0.3s;
}

.tab-item.active {
  color: #4259ff;
  font-weight: bold;
  border-bottom: 2px solid #4259ff;
}
  
  /* tab内容区域 */
  .tab-content {
    flex: 1;
    overflow-y: auto;
    padding: 0px;
  }
  
  .placeholder {
    color: #888;
    text-align: center;
    margin-top: 50px;
    font-size: 18px;
  }
  </style>
  