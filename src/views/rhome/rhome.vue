  ...
  <banner
      :bannerData="[
        ...
        },
        {
          ...
          des1: [
            { i18nKey: 'setting.banner.through' },' MetaMask ',{ i18nKey: 'setting.banner.withdraw' }  
          ],
    ...
    <ChainChangeDialog v-if="showNetworkModal" 
      @close="showNetworkModal = false" 
      @select-network="handleSelectNetwork"
      @add-zdynetwork="handleAddZdyNetwork"
    />
    ...
    <AddNetworkZdyPopup v-if="showAddPopup" @close="closePopup" />
       ...
       <!-- 切换账号弹框 -->
       <AcountChangeDialog v-if="selectUVueShow" 
        @closeFn="selectUVueShow = false"
        @close="selectUVueShow = false"
        @accountRefresh="accountRefresh"
      
        />

        <SendDialog
      v-if="SendDialogModal"
      :asset="selectedAsset"
      @cancelclick="SendDialogModal = false"
      @nextclick="onNextSend"
      
      @selectSendAccount="onSelectSendAccount"
    />
