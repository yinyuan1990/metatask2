	...
	methods: {
		onItemClick(item, index) {
			...
			if (routeMap[item.title]) {
				...
				if (item.title === this.$t('setting.featureRequests')) window.location.href = 'https://community.metamask.io/c/feature-requests-ideas/';
	...
	<div class="setting">
		...
		<div class="list-container">
			<div class="list" v-for="(item, i) in list" :key="i" @click="onItemClick(item, i)">
				...
				<img src="@/static/icon/arrow-right.svg" alt="" class="arrow-right" v-if="item.title !== $t('setting.featureRequests') && item.title !== $t('setting.contactSupport')" />
			</div>
