  ...
  <div class="page trade-page p-13 f-14">
    <div class="flex-r f-13">
      <div class="flex text-center flex-col items-center justify-center">
        <div class="f-16 bold">{{ $t('trade.buyAmount') }}</div>
        <div class="mt-5 c-9">{{ getNameByChainId2() }}</div>
      </div>
      <div class="pl-10 c-main" @click="backRoot()">{{ $t('common2.cancel') }}</div>
    </div>
    ...
    <div class="flex-auto">
      ...
      <p class="c-6">{{ $t('trade.youWantToBuy') }}</p>
      <div class="btn-border flex-r">
        ...
        <div class="flex text-line1 bold pl-10">{{ getName() }}</div>
        <div class="bold pl-10">{{ getName1() }}</div>
        <img class="icon-arrow" alt="" src="@/static/icon/arrow-down.svg" />
      </div>
      ...
      <div class="btn-border flex-r">
        ...
        <input class="flex f-16 bold price-inp" v-model="price" type="number" />
        <div>USD</div>
        <img class="icon-arrow" alt="" src="@/static/icon/arrow-down.svg" />
      </div>

      <p class="pt-10 c-6">{{ $t('trade.updatePaymentMethod') }}</p>
      <div class="btn-border">
        <div class="pay-way flex-r ptb-10">
          ...
          <div class="bold pl-10">{{ $t('common2.change') }}</div>
          <img class="icon-arrow" alt="" src="@/static/icon/arrow-down.svg" />
        </div>
    ...
    <van-button class="g-radius mt-13" type="info" :disabled="!price" @click="getPrice()">
      {{ $t('trade.getQuote') }}
    </van-button>
  </div>
</template>
