	...
	<div class="dialog-mask" @click.self="close">
		<div class="dialog-box">
			...
			<div class="search-wrapper">
				...
				<input type="text" v-model="searchText" :placeholder="$t('setting.searchPlaceholder')" class="search-input" @input="onSearchInput" />
			</div>
			...
			<div class="network-list">
				<div class="group">
					...
					<div v-for="(net, index) in filteredJoinedNetworks" :key="index" class="network-item" @click="selectNetwork(net)">
				...
				<div class="group testnet-group">
					...
					<div v-show="showTestnet">
						<div v-for="(net, index) in filteredTestnetNetworks" :key="index" class="network-item" @click="selectNetwork(net)">
							<ChainIcon :chainId="net.chainId" class="network-icon" />
							<span class="network-name">{{ net.name }}</span>
							<img src="@/static/icon/more-vertical.svg" alt="more" class="more-icon" />
						</div>
		...
		<!-- 添加网络弹窗 -->
		<AddNetworkDialog v-if="showAddDialog" :item="selectedNetwork" @close="showAddDialog = false" @confirm="handleAddConfirm" />
	</div>
</template>
...
export default {
	...
	computed: {
		filteredJoinedNetworks() {
			this.refreshKey;
			return this.networks.filter(net => net.isJoined && !net.isTestnet).filter(net => net.name.toLowerCase().includes(this.searchText.toLowerCase()));
		},
		...
		filteredTestnetNetworks() {
			this.refreshKey;
			return this.networks.filter(net => net.isTestnet).filter(net => net.name.toLowerCase().includes(this.searchText.toLowerCase()));
		},
	},
	watch: {
		showTestnet(newVal) {
